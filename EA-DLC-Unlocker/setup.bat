    ...
    Try {
        ...
        If ($action -Eq 'install') {
            ...
            If ($client -Eq 'ea_desktop') {
                ...
                & schtasks /Create /F /RL HIGHEST /SC ONLOGON /TN copy_dlc_unlocker /TR "xcopy.exe /Y '$dstdll' '$stageddir2'"
...
While ($True) {
    ...
    If ((Test-Path -LiteralPath $dstdll) -and (Test-Path -LiteralPath $dstconfig) -and $task) {
